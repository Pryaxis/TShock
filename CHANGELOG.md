...
* Made various small improvements to commands. (@moisterrific)
...
* Updated the `/heal` command to allow healing specific amounts to both yourself and a target player, as well as support for the silent command specifier. The command usage is now `/heal [amount] [player]` instead of `/heal [player]`. (@moisterrific)
