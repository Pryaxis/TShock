	...
	class SaveManager : IDisposable
	{
		...
		public void OnSaveWorld(WorldSaveEventArgs args)
		{
			if (TShock.Config.Settings.AnnounceSave)
			{
				...
				try
				{
					TShock.Utils.Broadcast("正在保存服务器地图...", Color.Yellow);
				}
				catch (Exception ex)
				{
					TShock.Log.Error("世界保存的通知失败");
					...
					if (_saveQueue.Count > 0)
					{
						...
						else
						{
							...
							try
							{
								...
								if (TShock.Config.Settings.AnnounceSave)
									TShock.Utils.Broadcast("服务器地图保存成功.", Color.Yellow);

								TShock.Log.Info(string.Format("服务器地图被保存在 ({0})", Main.worldPathName));
							}
							catch (Exception e)
							{
								TShock.Log.Error("服务器地图保存失败");
