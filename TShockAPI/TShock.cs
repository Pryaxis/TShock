	...
	public class TShock : TerrariaPlugin
	{
		...
		public override void Initialize()
		{
			...
			try
			{
				...
				Log.ConsoleInfo("TShock {0} ({1}) 汉化版 Beta 1 正在运行。", Version, VersionCodename);
				...
				if (Config.Settings.AutoSave)
					Log.ConsoleInfo(GetString("自动保存 已开启"));
				else
					Log.ConsoleInfo(GetString("自动保存 已关闭"));
				if (Backups.Interval > 0)
					Log.ConsoleInfo(GetString("自动备份 已开启"));
				else
					Log.ConsoleInfo(GetString("自动备份 已关闭"));

				Initialized?.Invoke();

				Log.ConsoleInfo("欢迎使用TShock 初始化完毕");
				Log.ConsoleInfo("TShock 完全免费且开源~");
		...
		private void OnConnect(ConnectEventArgs args)
		{
			...
			if (Geo != null)
			{
				...
				if (code == "A1")
				{
					...
					{
						player.Kick("禁止使用代理。", true, true, null, false);
						args.Handled = true;
						return;
					}
				}
		...
		private void OnJoin(JoinEventArgs args)
		{
			...
			{
				player.Kick("客户端异常，请重新连接。", true, true, null, false);
				args.Handled = true;
				return;
			}
		...
		private void ServerHooks_OnCommand(CommandEventArgs args)
		{
			...
			if (args.Command == "autosave")
			{
				...
				Log.ConsoleInfo("自动保存 " + (Config.Settings.AutoSave ? "已开启" : "已关闭"));
			}
