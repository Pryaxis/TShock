		...
		private void HandleCommandLine(string[] parms)
		{
			...
			CliParser
				...
				.AddFlag("-config", (cfg) =>
					{
						...
						{
							_logger.LogDebug(GetString("Loading dedicated config file: {0}", cfg));
				...
				.AddFlag("-port", (p) =>
					{
						...
						if (int.TryParse(p, out port))
						{
							...
							_logger.LogDebug(GetString("Listening on port {0}.", port));
						}
					})
				...
				.AddFlag("-worldname", (world) =>
					{
						if (!string.IsNullOrWhiteSpace(world))
						{
							...
							_logger.LogDebug(GetString("World name will be overridden by: {0}", world));
						}
					})
				...
				.AddFlag("-ip", (ip) =>
					{
						...
						if (IPAddress.TryParse(ip, out addr))
						{
							...
							_logger.LogDebug(GetString("Listening on IP {0}.", addr));
						}
						else
						{
				...
				.AddFlag("-worldevil", (value) =>
				{
					...
					_logger.LogDebug(GetString("New worlds will be generated with the {0} world evil type!", value));
